...
# Load your CSV
df = pd.read_csv("/Users/judyz/Desktop/copy/build/hits_per_event.csv")

# Full LXe bounds from construction.cc
x_min, x_max = -35, 35  # mm
z_min, z_max = -25, 25  # mm
n_bins = 38  # grid resolution
...
H, _, _ = np.histogram2d(df["x(mm)"], df["z(mm)"], bins=[x_bins, z_bins], weights=df["LCE (%)"])
counts, _, _ = np.histogram2d(df["x(mm)"], df["z(mm)"], bins=[x_bins, z_bins])
...
plt.pcolormesh(x_bins, z_bins, avg_LCE.T, shading='auto', cmap='viridis')
plt.colorbar(label="LCE (%)")
plt.xlabel("X (mm)")
plt.ylabel("Z (mm)")
